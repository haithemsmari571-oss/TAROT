...
# NOTE: POST /test-email and /test-email-raw were removed deliberately. They
# took an arbitrary recipient as a query parameter, had no authentication and no
# rate limit, and sent through the production SMTP account — an open relay from
# a domain with aligned DKIM/DMARC. Diagnose mail with a local Mailpit run
# (docker-compose brings one up) rather than reinstating them.
