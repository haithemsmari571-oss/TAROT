  ...
  return (
    <main className="articles-shell">
      ...
      <header className="articles-hero">
        <div className="articles-hero__glow" aria-hidden="true" />
        <nav aria-label="Breadcrumb"><Link to="/">Home</Link><span>/</span><span>Articles</span></nav>
        <span className="article-kicker">The Ask Valentina library</span>
        <h1>{routeCategory ?? "Guidance for the questions that stay with you."}</h1>
        <p>Thoughtful, grounded reading on numerology, tarot, relationships and spiritual life.</p>
        <form className="article-search" onSubmit={submitSearch} role="search">
      ...
      <nav className="article-filters" aria-label="Article categories">
        <Link className={!routeCategory ? "active" : ""} to={query ? `/articles/?q=${encodeURIComponent(query)}` : "/articles/"}>All</Link>
        {Object.entries(categoryPaths).map(([slug, label]) => (
          <Link
            className={routeCategory === label ? "active" : ""}
            key={slug}
      ...
      {loading ? (
        <section className="article-state" aria-live="polite"><div className="article-loader" /><h2>Opening the library…</h2></section>
      ) : error && !items.length ? (
        <section className="article-state" role="alert"><h2>We hit a quiet patch.</h2><p>{error}</p><button onClick={resetSearch}>Try the library again</button></section>
      ) : !items.length ? (
        <section className="article-state">
          <span aria-hidden="true">✦</span><h2>No articles matched that search.</h2>
        ...
        <>
          {!query && data?.featured?.length ? (
            <section className="articles-section">
              <div className="articles-section__heading"><span>Chosen for you</span><h2>Featured</h2></div>
          ...
          <section className="articles-section">
            <div className="articles-section__heading">
              <span>{query ? `${data?.total ?? 0} result${data?.total === 1 ? "" : "s"}` : "Fresh from the library"}</span>
              <h2>{query ? `Search results for “${query}”` : "Recent articles"}</h2>
            </div>
            ...
            {data?.has_more ? <button className="article-load-more" onClick={loadMore} disabled={loadingMore}>{loadingMore ? "Loading…" : "Load more articles"}</button> : null}
...
export function ArticleDetail() {
  ...
  if (missing) return <main className="article-reading-shell article-state"><Seo meta={{ path: `/articles/${slug}`, title: "Article not found | Ask Valentina", description: "This article is not available.", robots: "noindex,nofollow" }} /><h1>Article not found</h1><Link to="/articles/">Return to Articles</Link></main>;
  if (!item) return <main className="article-reading-shell article-state"><Seo meta={{ path: `/articles/${slug}`, title: "Opening article | Ask Valentina", description: "Opening this Ask Valentina article.", robots: "noindex,nofollow" }} /><div className="article-loader" /><p>Opening article…</p></main>;
    ...
    <main className="article-reading-shell">
      ...
      <article className="article-page">
        <header className="article-page__header">
          <span className="article-kicker">{item.category}</span>
        ...
        <div className="article-reading-layout">
          ...
          <div className="article-reading-main">
            ...
            <div className="article-share"><span>Found this useful?</span><button onClick={share}>Share article</button>{shareNotice && <small role="status">{shareNotice}</small>}</div>
      ...
      {item.related?.length ? (
        <section className="articles-section article-related">
          <div className="articles-section__heading"><span>Keep reading</span><h2>Related articles</h2></div>
