...
const TAROT_ASSETS = [
  ...
  "33.png", "34.png", "35.png", "36.png", "37.png", "38.png", "39.png"
];
...
const CONSTELLATION_DATA = [
  ...
  },
  {
    ...
    stars: [[50,10], [50,40], [50,90], [10,40], [90,40]]
  }
];
...
const navigate = useNavigate();
  ...
  const cards = useMemo(() => {
    return TAROT_ASSETS.map((src, i) => {
      ...
      const xPos = side === 1 ? Math.random() * 20 + 80 : Math.random() * 20 - 15; 
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center pt-24 px-6">
          ...
          {constellations.map((con, i) => (
            <motion.svg
              ...
              style={{
                ...
                rotate: con.rotate,
                scale: con.scale,
                x: useTransform(scrollYProgress, [0, 1], [0, -windowSize.width * con.speed]),
                y: useTransform(springY, [-0.5, 0.5], [-30, 30]),
                opacity: useTransform(scrollYProgress, [0, 0.4], [con.opacity, 0])
              }}
            >
              {/* Constellation Lines */}
              <motion.path
        ...
        <div className="absolute inset-0 pointer-events-none z-10">
          {cards.map((card, i) => {
            ...
            const scrollX = useTransform(scrollYProgress, [0, 1], [0, direction * windowSize.width * card.scrollSpeed]);
            const mouseMoveX = useTransform(springX, [-0.5, 0.5], [-card.mouseFactor, card.mouseFactor]);
            const mouseMoveY = useTransform(springY, [-0.5, 0.5], [-card.mouseFactor, card.mouseFactor]);
            const tilt = useTransform(springX, [-0.5, 0.5], [card.rotate - 10, card.rotate + 10]);
