...
const Billing = () => {
    ...
    const fetchApiOptions = async () => {
      try {
        ...
        const mappedOptions = data.map((option: any, idx: number) => ({
          ...
          price: option.price_cents || option.price || (option.points ? option.points * 10 : 0),
        }));
  ...
  const pointsPackages: BillingPackage[] = useMemo(() => {
    ...
    return [
      { id: "starter", points: 5, amount: 5, price: pricePerPoint * 5, label: "Starter" },
      { id: "basic", points: 20, amount: 20, price: pricePerPoint * 20, label: "Basic" },
      { id: "popular", points: 50, amount: 50, price: pricePerPoint * 50, label: "Popular" },
      { id: "pro", points: 100, amount: 100, price: pricePerPoint * 100, label: "Pro" },
      { id: "premium", points: 250, amount: 250, price: pricePerPoint * 250, label: "Premium" },
      { id: "elite", points: 500, amount: 500, price: pricePerPoint * 500, label: "Elite" },
    ];
  ...
  const formatCurrency = (amount: number) =>
    new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(amount / 1000);
  ...
  const handlePurchase = async (pkg: BillingPackage) => {
    ...
    try {
      ...
      await createCheckoutSession({
        ...
        return_url: returnUrl || `${window.location.origin}/billing?status=success`,
      });
    } catch (err) {
  ...
  const selectedPackage = pointsPackages.find((pkg) => pkg.id === selectedPackageId);
  const bestValuePackage = pointsPackages.reduce<BillingPackage | null>((best, pkg) => {
    if (pkg.points <= 0 || pkg.price <= 0) return best;
    if (!best || pkg.price / pkg.points < best.price / best.points) return pkg;
    return best;
  }, null);
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1280px]">
        ...
        <div className="mb-8 grid grid-cols-1 gap-5 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
          ...
          >
            ...
            <div className="relative">
              <div className="mb-6 flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 backdrop-blur-xl">
                <Icon icon="ph:sparkle-fill" className="text-sm" style={{ color: COLORS.primary }} />
              ...
              <p className="mt-5 max-w-2xl text-sm leading-7 text-white/55 sm:text-base">
                Choose a tier, complete secure Stripe checkout, and keep your reading credits ready when the moment calls.
              </p>
              ...
              <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <div className="mb-2 flex items-center gap-2 text-white/40">
                    ...
                    <span className="text-[8px] font-black uppercase tracking-[0.22em]">Checkout</span>
                  </div>
                  <div className="text-sm font-black uppercase text-white">Stripe Secure</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <div className="mb-2 flex items-center gap-2 text-white/40">
                    <Icon icon="solar:history-bold-duotone" className="text-lg" />
                    <span className="text-[8px] font-black uppercase tracking-[0.22em]">Ledger</span>
                  </div>
                  <div className="text-sm font-black uppercase text-white">{transactionsData?.total ?? 0} Records</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <div className="mb-2 flex items-center gap-2 text-white/40">
                    <Icon icon="solar:shield-check-bold-duotone" className="text-lg" />
                    <span className="text-[8px] font-black uppercase tracking-[0.22em]">Status</span>
                  </div>
                  <div className="text-sm font-black uppercase text-white">Active</div>
                </div>
              </div>
            </div>
          </section>
          ...
          >
            <div className="absolute inset-x-8 top-0 h-px" style={{ backgroundColor: COLORS.primary }} />
            <div className="relative flex h-full flex-col justify-between">
              <div>
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-[0.28em] text-white/40">Available Balance</p>
                    <p className="mt-2 text-sm text-white/45">Ready for readings</p>
                  </div>
                  <div
                    className="flex h-12 w-12 items-center justify-center rounded-2xl border"
                    style={{ backgroundColor: `${COLORS.primary}12`, borderColor: `${COLORS.primary}24` }}
                  >
                    <Icon icon="ph:sparkle-fill" className="text-2xl" style={{ color: COLORS.primary }} />
                  </div>
                </div>
                ...
                <div className="flex items-end gap-3">
                  ...
                  <span className="pb-2 text-sm font-black uppercase tracking-[0.22em]" style={{ color: COLORS.primary }}>
                    Points
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8 space-y-3">
                <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3">
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40">Best Value</span>
                  <span className="text-sm font-black text-white">{bestValuePackage?.label ?? "Loading"}</span>
                </div>
                <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3">
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40">Selected</span>
                  <span className="text-sm font-black text-white">{selectedPackage?.label ?? "Choose Tier"}</span>
                </div>
              </div>
            </div>
          </aside>
            ...
            <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
              <div>
                <div className="mb-3 flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5">
                  <Icon icon="solar:bag-smile-bold-duotone" style={{ color: COLORS.primary }} className="text-base" />
                  <span className="text-[9px] font-black uppercase tracking-[0.24em] text-white/45">Stardust Tiers</span>
                </div>
                <h2
                ...
                <p className="mt-2 max-w-xl text-sm leading-6 text-white/45">
                  The same live tiers from the Stardust modal, tuned for faster comparison.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 p-2">
                ...
                >
                  <Icon icon="ph:caret-left-bold" className="text-xl" style={{ color: COLORS.primary }} />
                </button>
                ...
                <button
                  ...
                  className="flex h-11 w-11 items-center justify-center rounded-xl transition-all hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-25"
                >
                  <Icon icon="ph:caret-right-bold" className="text-xl" style={{ color: COLORS.primary }} />
                </button>
              </div>
            </div>
              ...
              >
                {pointsPackages.map((pkg, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative flex h-full cursor-pointer flex-col overflow-hidden rounded-[24px] p-5 transition-all duration-300 group sm:p-6 ${
                        ...
                        style={{
                          ...
                          borderColor: isSelected || isPopular ? `${COLORS.primary}66` : undefined,
                      ...
                      >
                        ...
                        <div className="relative z-10 flex h-full flex-col">
                          ...
                          <div className="mb-6 flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1 text-[9px] font-bold uppercase tracking-[0.16em] text-white/35">
                                {formatCurrency(pkg.price / Math.max(pkg.points, 1))} / point
                              </div>
                            </div>
                            {isPopular && (
                              <div
                          ...
                          <div className="mb-7">
                            ...
                            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.neutralGray }}>
                              <Icon icon="ph:sparkle-fill" style={{ color: COLORS.primary }} />
                              Points Included
                            </div>
                          </div>
                          ...
                          <div className="mb-auto rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                            <div className="text-[9px] font-black uppercase tracking-[0.2em] text-white/35">Total</div>
                            <div className="mt-1 text-3xl font-black sm:text-4xl" style={{ color: COLORS.primary }}>
        ...
        {combinedError && (
          <div
            ...
            style={{ backgroundColor: "rgba(248, 113, 113, 0.1)", boxShadow: "0 18px 44px rgba(0,0,0,0.24)" }}
          >
        ...
        >
          ...
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="mb-3 flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5">
                <Icon icon="solar:history-bold-duotone" style={{ color: COLORS.primary }} className="text-base" />
                <span className="text-[9px] font-black uppercase tracking-[0.24em] text-white/45">Ledger</span>
              </div>
              <h2
                className="text-3xl font-black uppercase tracking-normal sm:text-4xl"
                style={{ color: COLORS.neutralWhite }}
            ...
            {transactions.length > 0 && (
              ...
              >
                <span className="text-[9px] font-black uppercase tracking-[0.2em]" style={{ color: COLORS.primary }}>
          ...
          {combinedLoading && transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20">
              <Icon icon="svg-spinners:3-dots-fade" className="text-5xl mb-4" style={{ color: COLORS.primary }} />
              <p className="text-sm opacity-50" style={{ color: COLORS.neutralGray }}>
            ...
            <div
              ...
              style={{ backgroundColor: "rgba(255,255,255,0.035)", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}
            >
              <Icon
              ...
              />
              <h3 className="text-lg sm:text-xl font-black uppercase mb-2" style={{ color: COLORS.neutralGray }}>
                No Transactions Yet
              </h3>
              <p className="text-xs sm:text-sm opacity-50" style={{ color: COLORS.neutralGray }}>
                ...
                {transactions.map((transaction: Transaction) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ backgroundColor: "rgba(255,255,255,0.035)", boxShadow: "0 8px 30px rgba(0,0,0,0.18)" }}
                    >
                      {/* Row hover glow */}
                      <div
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                        style={{ background: `linear-gradient(135deg, ${typeDisplay.color}05 0%, transparent 70%)` }}
                      />
                      ...
                      <div className="relative z-10 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center sm:gap-6">
                        ...
                        <div className="flex w-full items-center gap-3 sm:w-auto sm:gap-5">
                          ...
                          >
                            <Icon icon={typeDisplay.icon} className="text-xl sm:text-3xl" style={{ color: typeDisplay.color }} />
                          </div>
              ...
              {transactionsData && transactionsData.pages > 1 && (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ backgroundColor: `${COLORS.dark}cc`, borderColor: `${COLORS.primary}20` }}
                  >
                    {/* Prev */}
                    <button
                    ...
                    >
                      <Icon icon="solar:alt-arrow-left-linear" className="text-xl" style={{ color: COLORS.primary }} />
                    </button>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 px-1 sm:px-2">
                      {Array.from({ length: transactionsData.pages }, (_, i) => i + 1)
                        .filter((page) =>
                          page === 1 || page === transactionsData.pages || Math.abs(page - currentPage) <= 1
                        )
                        .map((page, idx, arr) => {
                          ...
                          return (
                            <div key={page} className="flex items-center gap-1 sm:gap-2">
                              {showEllipsis && (
                                <span className="px-1 sm:px-2 text-[10px] sm:text-xs" style={{ color: COLORS.neutralGray }}>
                                  …
                                </span>
                              )}
                              <button
                                ...
                                style={{
                                  ...
                                  color: currentPage === page ? COLORS.dark : COLORS.neutralGray,
                                  borderColor: currentPage === page ? COLORS.primary : "transparent",
                                  boxShadow: currentPage === page ? `0 0 20px ${COLORS.primary}40` : "none",
                                }}
                              >
                                {page}
                              </button>
                    ...
                    <button
                      ...
                      className="p-2 sm:p-3 rounded-xl transition-all hover:bg-white/10 disabled:opacity-20 disabled:cursor-not-allowed"
                    >
                      <Icon icon="solar:alt-arrow-right-linear" className="text-xl" style={{ color: COLORS.primary }} />
                    </button>
                  </div>
                </div>
              )}
          ...
          >
            ...
            <div className="text-center relative z-10">
              <div
                ...
                style={{ backgroundColor: `${COLORS.primary}20`, border: `3px solid ${COLORS.primary}40` }}
              >
                <Icon icon="solar:check-circle-bold-duotone" className="text-7xl" style={{ color: COLORS.primary }} />
                <div
              ...
              <p className="text-sm sm:text-base mb-6 sm:mb-8 font-light px-2" style={{ color: COLORS.neutralGray }}>
              ...
              <button
                ...
                style={{ backgroundColor: COLORS.primary, color: COLORS.dark, boxShadow: `0 10px 40px ${COLORS.primary}40` }}
              >
        ...
        >
          ...
          >
            ...
            <div className="text-center relative z-10">
              <div
                className="w-20 h-20 sm:w-28 sm:h-28 rounded-full flex items-center justify-center mx-auto mb-6 sm:mb-8 relative"
                style={{ backgroundColor: "rgba(248,113,113,0.15)", border: "3px solid rgba(248,113,113,0.3)" }}
              >
                <Icon icon="solar:close-circle-bold-duotone" className="text-5xl sm:text-7xl" style={{ color: "#F87171" }} />
                <div
                  className="absolute inset-0 rounded-full"
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-black uppercase mb-3 sm:mb-4" style={{ color: "#F87171" }}>
                Payment Failed
              </h2>
              <p className="text-sm sm:text-base mb-6 sm:mb-8 font-light px-2" style={{ color: COLORS.neutralGray }}>
              ...
              <button
                onClick={closeErrorModal}
                className="w-full py-3 sm:py-4 rounded-2xl font-black text-[10px] sm:text-[11px] uppercase tracking-widest transition-all relative overflow-hidden group"
                style={{ backgroundColor: "#F87171", color: COLORS.dark, boxShadow: "0 10px 40px rgba(248,113,113,0.4)" }}
              >
